...
- **Rotate** `JWT_SECRET`, `LORE_BOOTSTRAP_TOKEN` and `SESSION_SECRET` if they were
  ever shared or copied from an example.
...
- Postgres is **not** published to the host (internal compose network only); only
  Caddy (80/443) and the web app (3001) are exposed.
