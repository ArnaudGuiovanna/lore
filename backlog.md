...
- `B-08` : backend implemente (profil legal tenant migration 000007, export Qualiopi JSON par cohorte). Front : vague D (agent en cours).
- `B-10` : backend implemente (documents contractuels versionnes root_id, scope apprenant). Front : vague D (agent en cours).
- `B-11` : backend implemente (enquetes HOT/COLD, reponses 1..5 uniques, reclamations workflow). Front : vague D (agent en cours).
- `B-13` : implemente backend (positionnement archivable = premiere evaluation corrigee par concept, garde IDOR).
- `B-14` : backend implemente (effacement RGPD memoire+Postgres, anonymisation conditionnee aux memberships actives, capacite erase_data admin uniquement). Front : vague D (agent en cours).
...
- `B-16` : backend implemente (review_status sur generated_contents, REJECTED masque des lectures persistees, file `/content-review`, migration 000010). Front file de revue : a venir (vague D').
- `B-17` : backend implemente (ressources FICHIER bytea <= 20 MiB / LIEN, scope cohorte+tenant, download streame, migration 000012). Front : a venir (vague D').
- `B-18` : backend implemente (annonces cohorte/tenant, narrowing apprenant, migration 000010). Front annonces + reset mdp self-service : a venir.
...
- `B-28` : backend implemente. Textes legaux versionnes (CGU/CONFIDENTIALITE/MENTIONS, version=max+1, migration 000011), consentement idempotent a l'identite du jeton, registre staff / vue self apprenant. Front consentement : vague D (agent en cours).
