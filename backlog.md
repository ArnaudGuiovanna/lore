...
- `B-07` : implemente. Pause/reprise persistees (`paused_seconds`/`paused_at`, migration 000003), endpoints `/activities/{id}/pause|resume`, pause auto du workbench quand l'onglet est masque, repli des pauses ouvertes a la completion, agregation et CSV excluant les pauses, heures affichees dans les vues formateur (sante du groupe) et admin (vue d'ensemble) avec export CSV. Le workbench declenche desormais `/start` (le temps ne s'accumulait pas hors seed).
- `B-12` : implemente. CRUD/archive users/programs/cohorts/enrollments, sessions planifiees (backend + UI admin Sessions : liste/creation/archivage), audit admin, migration RLS. Import CSV en masse (`/api/admin/import-learners` + UI Import CSV : provisioning credential + membership + inscription cohorte + envoi e-mail optionnel, rapport par ligne telechargeable). Exports CSV : temps de formation et progression/completion par apprenant (`progress.csv` backend, threshold maitrise runtime), liens dans vues admin et formateur.
- `B-21` : implemente pour le socle vise. `/metrics` protege par token, compose prod genere/exige `LORE_METRICS_TOKEN`, Caddy headers, `SECURITY.md`, et desormais service `db-backup` (pg_dump -Fc quotidien, retention configurable, volume `backups`). Reste : restauration automatisee testee en CI.
- `B-08` : implemente. Profil legal tenant (formulaire admin Conformite > Profil OF) + export Qualiopi JSON telechargeable par cohorte.
- `B-10` : implemente. Documents contractuels versionnes (gabarits FR par type cote admin, page Documents apprenant scopee).
- `B-11` : implemente. Enquetes HOT/COLD (creation admin, reponse apprenant etoiles 1..5 + texte, moyennes + verbatims) ; registre des reclamations avec workflow statut/resolution + depot apprenant.
- `B-13` : implemente. Positionnement archivable backend (premiere evaluation corrigee par concept) + vue formateur (date/concept/score initial par apprenant).
- `B-14` : implemente. Effacement RGPD backend (anonymisation conditionnee aux memberships actives, capacite erase_data admin) + UI admin avec double confirmation par saisie du nom.
- `B-15` : implemente backend. Dossiers de financement (CPF/OPCO/France Travail/EMPLOYEUR/AUTOFINANCEMENT/AUTRE, statuts, montants, migration 000011) + export BPF annuel (`/bpf-export?year=` : produits par origine, stagiaires, heures reellement tracees). Connecteurs EDOF/Kairos non implementes : ce modele est leur source de synchronisation future.
- `B-16` : implemente. Backend (review_status, REJECTED masque, file `/content-review`, migration 000010) + section formateur Curation (filtres, provenance, approuver/rejeter avec note obligatoire au rejet).
- `B-17` : implemente. Backend (FICHIER bytea <= 20 MiB / LIEN, scope cohorte+tenant, download streame, migration 000012) + section formateur Ressources (upload base64, ciblage) + page Ressources apprenant (telechargement streame). SCORM/H5P toujours hors perimetre (AI-first).
- `B-18` : implemente pour annonces. Backend (migration 000010, narrowing apprenant) + section formateur Annonces + bandeau des 3 dernieres sur l'accueil apprenant. Reste : messagerie interne, reset mot de passe self-service (l'admin reinitialise via credentials).
- `B-20` : implemente pour OIDC et webhooks. OIDC complet cote verification (discovery .well-known, JWKS par kid, rotation rate-limitee, validation iss/aud/exp, claims custom tenant_id/role requis, env `OIDC_ISSUER`/`OIDC_AUDIENCE`) en fallback du service local. Webhooks signes HMAC-SHA256 sur l'outbox (`X-LORE-Signature`, at-least-once, exclusif NATS via `LORE_WEBHOOKS=on`, migration 000013, secret servi une seule fois). Reste : SAML, LTI, xAPI/LRS.
- `B-23` : implemente. Invitations par code (migration 000005, lookup public, redeem bootstrap atomique) + page publique `/join/[code]` (rate-limit, compte choisi par l'utilisateur) + UI admin Invitations (lien copiable, statuts, revocation). Import CSV en masse via B-12.
- `B-24` : implemente. Modules editoriaux par syllabus (positions, prerequis DAG, migration 000004), chemin apprenant calcule sur l'evidence runtime (verrouille/en cours/termine), gating de PlanNext via `allowed_concept_ids`, pages Parcours apprenant + editeur formateur.
- `B-25` : implemente. Sessions planifiees (presentiel/visio video_url), export iCalendar `.ics`, agenda apprenant, lisible apprenant.
- `B-26` : implemente. Banque de questions (QCM/reponse courte, cle jamais servie a l'apprenant, scoring serveur au submit), devoirs par cohorte avec echeance, soumission apprenant (re-rendu jusqu'a notation, 409 ensuite, lecture de sa propre copie), correction manuelle (note 0..1 + feedback) reliee au BKT via evidence MANUAL_GRADE ; le runtime prefere les questions de banque du concept en phase d'evaluation. Fronts formateur (Evaluations 3 volets) et apprenant (Devoirs) livres.
- `B-27` : implemente pour le perimetre cible. Matrice de capacites backend (manage_users/enrollments/sessions/documents/finance/legal/quality, erase_data, configure_technical), role `GESTIONNAIRE` (administratif sans configuration technique ni conception pedagogique, memberships limites a LEARNER), 27 gardes admin migrees vers `authorizeCapability`. Reste : multi-roles par utilisateur, front gestionnaire.
- `B-28` : implemente. Textes legaux versionnes (migration 000011), consentement idempotent a l'identite du jeton, registre admin (Conformite > Textes legaux) + banniere de consentement persistante non bloquante cote apprenant.
- `B-15` (front) : implemente. Admin Conformite > Financements (dossiers €<->cents, statuts) + encart BPF annuel telechargeable.
- `B-19` : implemente pour le socle. `@axe-core/playwright` en CI e2e (3 roles x pages cles, wcag2a/aa) : 0 violation critical ; serious restantes = color-contrast des tokens typographiques discrets (.kicker/.quiet), a traiter via la palette. Declaration d'accessibilite : a rediger.
- `B-20` (xAPI) : implemente en complement — format de livraison `xapi` sur les webhooks (statements ADL minimal valides vers un LRS). Reste : SAML, LTI.
- `B-22` : implemente. Alertes (Inactivity 14j/30j) avec actions recommandees cote formateur + tuile admin, progression/completion par apprenant et par cohorte, exports CSV temps + progression, export Qualiopi.
- `B-27` (front) : implemente. GESTIONNAIRE invitable (LEARNER uniquement depuis ce role), console admin filtree (jamais LLM/outbox/RGPD), nav reduite, middleware /admin durci.
- `UX-03` : implemente pour les deux parcours canoniques (admin bout en bout jusqu'a l'export Qualiopi ; apprenant parcours -> activite -> devoirs/documents/agenda) en e2e Playwright. Onboarding gestionnaire/formateur : iterations futures.
